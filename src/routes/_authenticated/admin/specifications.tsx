    ...
    <div className="max-w-5xl mx-auto py-8 px-6">
      ...
      <div className="flex items-center justify-between gap-4 mb-8 flex-wrap">
        <div>
          ...
          <h1 className="text-3xl font-serif text-[#2d2d2d]">Specifications</h1>
...
function SpecCard({ row, onEdit, onDelete }: { row: Spec; onEdit: () => void; onDelete: () => void }) {
  ...
  return (
    <div className="bg-white rounded-lg border border-[#e8e4dd] p-4 flex items-start gap-4">
      ...
      <div className="min-w-0 flex-1">
        <div className="flex items-center gap-2">
          <h3 className="font-serif text-[#2d2d2d]">{row.group_name}</h3>
          <span className="text-xs px-1.5 py-0.5 rounded bg-[#f0ebe3] text-[#5c4d3c]">#{row.sort_order}</span>
        </div>
        <p className="text-sm text-[#5c4d3c] mt-1">{row.detail}</p>
      </div>
