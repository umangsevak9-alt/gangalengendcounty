...
const videoSchema = z.object({
  ...
  aspect_ratio_mobile: z.enum(["16/9", "4/3", "1/1", "9/16", "21/9"]).default("9/16"),
...
export const upsertVideoSection = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const row = {
      ...
      poster_path: data.poster_path ?? null, aspect_ratio: data.aspect_ratio,
      aspect_ratio_mobile: data.aspect_ratio_mobile, is_active: data.is_active,
    };
    if (data.id) {
